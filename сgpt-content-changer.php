...
add_action('wp_ajax_cgptcc_create_post', 'cgptcc_create_post');
add_action('wp_ajax_nopriv_cgptcc_create_post', 'cgptcc_create_post');

function cgptcc_get_recent_posts() {
    $posts = array();
    
    // Retrieve current options
    $options = get_option('my_plugin_options');

    // Define your category ID and desired post status
    $category_id = $options['post_to_be_category']; // Replace with your category ID
    $post_status = $options['post_to_be_status']; // Replace with your desired post status

    $minutes_ago = $options['interval'];; // Replace with the desired number of minutes

    // Calculate the start and end date for your time period
    $end_date = current_time('mysql'); // Current date and time
    $start_date = date('Y-m-d H:i:s', strtotime("-{$minutes_ago} minutes")); // Dynamic start date

    // Create an array of query arguments
    $args = array(
        'cat' => $category_id,          // Category ID
        'post_status' => $post_status,  // Post status
        'date_query' => array(
            'after'     => $start_date, // Start date
            'before'    => $end_date,   // End date (current time)
            'inclusive' => true,        // Include posts within the specified period
        ),
    ...
    if ($query->have_posts()) {
        ...
        // Encode the posts array as JSON
...
add_action('wp_ajax_cgptcc_get_recent_posts', 'cgptcc_get_recent_posts');
add_action('wp_ajax_nopriv_cgptcc_get_recent_posts', 'cgptcc_get_recent_posts');

// Add an admin menu item for your settings page
function my_plugin_settings_menu() {
add_menu_page('AI Content Changer Settings', 'AI Content Changer Settings', 'manage_options', 'my-plugin-settings',
...
<div class="wrap">
    ...
    <div class="wrap">
        ...
        <form method="post">
            ...
            <input style="margin-bottom: 20px;" type="text" name="api_key"
                value="<?php echo esc_attr($options['api_key']); ?>" /><br />

            <label for="model">Model:</label>
            <input style="margin-bottom: 20px;" type="text" name="model"
                value="<?php echo esc_attr($options['model']); ?>" /><br />

            <label for="title_prompt">Title Prompt:</label>
            <input style="margin-bottom: 20px;" style="margin-bottom: 20px;" type="text" name="title_prompt"
                value="<?php echo esc_attr($options['title_prompt']); ?>" /><br />

            <label for="content_prompt">Content Prompt:</label>
            <textarea style="margin-bottom: 20px;"
                name="content_prompt"><?php echo esc_textarea($options['content_prompt']); ?></textarea><br />

            <label for="interval">Interval (minutes):</label>
            <input style="margin-bottom: 20px;" type="number" name="interval"
                value="<?php echo esc_attr($options['interval']); ?>" /><br />

            <!-- Select field for post status -->
            <label for="post_to_be_status">Posts to be Transformed Status:</label>
            <select style="margin-bottom: 20px;" name="post_to_be_status">
            ...
            <select style="margin-bottom: 20px;" name="post_to_be_category">
                <?php
                $args = array(
            ...
            <select style="margin-bottom: 20px;" name="post_status">
                <option value="publish"
            ...
            <select style="margin-bottom: 20px;" name="post_category">
                <?php
                $args = array(
                    'hide_empty' => false, // Set hide_empty to false to include empty terms
                    );
            ...
            <input style="margin-bottom: 40px;" type="submit" name="submit" value="Save Settings" />
        </form>
    </div>
